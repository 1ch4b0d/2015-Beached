	...
	void SpawnSingleElementDupes(EZParallaxObjectElement targetElement, Camera actualCamera, float screenWidth)
	{
		...
		for ( int k = 0; k < numRightDupes; ++k )
		{
			...
			if(dupeTargetElement.randomSpawnX)
			{
				...
				dupeTargetElement.initialRightSpawnDistanceScreenSpaceX = targetCenterPosScreenPt.x - actualCamera.GetComponent<Camera>().WorldToScreenPoint(dupeTargetElement.parallaxObject.position - (new Vector3(randomOffsetsList[offsetListShiftIdx], 0, dupeTargetElement.parallaxObject.position.z))).x;
			}
			else
			{
				objectOffsetVector = new Vector3(targetElement.spawnDistanceX + dupeTargetElement.meshWidth, 0, 0);
